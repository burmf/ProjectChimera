...
class VolBreakoutBitgetStrategy(TechnicalStrategy):
    ...
    def generate_signal(self, market_data: MarketFrame) -> Optional[Signal]:
        ...
        return Signal(
            ...
            strength=SignalStrength.STRONG if volume_confirmed else SignalStrength.MEDIUM,
            ...
            indicators_used={
                ...
                'breakout_level': breakout_long_level if signal_type == SignalType.BUY else breakout_short_level,
...
class MiniMomentumBitgetStrategy(TechnicalStrategy):
    ...
    def generate_signal(self, market_data: MarketFrame) -> Optional[Signal]:
        ...
        expected_move = abs(current_momentum) * current_price * 2  # 2x momentum as target
        ...
        return Signal(
            symbol=market_data.symbol,
            signal_type=signal_type,
            strength=SignalStrength.STRONG if abs(zscore) > 2.0 else SignalStrength.MEDIUM,
            price=Decimal(str(current_price)),
            timestamp=market_data.timestamp,
            strategy_name=self.name,
            confidence=confidence,
...
class LOBRevertBitgetStrategy(TechnicalStrategy):
    ...
    def calculate_order_flow_imbalance(self, orderbook) -> Optional[float]:
    ...
    def generate_signal(self, market_data: MarketFrame) -> Optional[Signal]:
        ...
        imbalance_rsi = self.calculate_rsi(historical_imbalances, self.imbalance_rsi_period)
        ...
        return Signal(
            ...
            indicators_used={
                ...
                'rsi_threshold': self.rsi_overbought if signal_type == SignalType.SELL else self.rsi_oversold,
                'spread_bps': float(market_data.orderbook.spread) / current_price * 10000,
...
class FundingAlphaBitgetStrategy(TechnicalStrategy):
    ...
    def __init__(self, config: StrategyConfig):
        ...
        self.oi_jump_threshold = self.params.get('oi_jump_threshold', 0.10)    # 10% OI increase
    ...
    def generate_signal(self, market_data: MarketFrame) -> Optional[Signal]:
        ...
        # In real implementation, this would be tracked over time
        historical_oi = [current_oi * (1 + np.random.normal(0, 0.05)) for _ in range(24)]
        ...
        if current_funding > self.funding_threshold:
            signal_type = SignalType.SELL
            confidence += min(0.2, (current_funding - self.funding_threshold) / self.funding_threshold)
        
        # High negative funding + OI jump = long perp (shorts paying, expect reversion)
        elif current_funding < -self.funding_threshold:
            signal_type = SignalType.BUY
            confidence += min(0.2, abs(current_funding + self.funding_threshold) / self.funding_threshold)
        ...
        if signal_type == SignalType.BUY:
            target_price = Decimal(str(current_price + funding_impact * 5))  # 5x funding as target
...
class BasisArbBitgetStrategy(TechnicalStrategy):
    ...
    def __init__(self, config: StrategyConfig):
        ...
        self.min_volume_ratio = self.params.get('min_volume_ratio', 0.1)      # 10% of perp volume
        self.convergence_target = self.params.get('convergence_target', 0.001) # 0.1% target
    ...
    def calculate_basis_premium(self, spot_price: float, perp_price: float) -> Optional[float]:
    ...
    def generate_signal(self, market_data: MarketFrame) -> Optional[Signal]:
        ...
        if premium > self.premium_threshold:
            signal_type = SignalType.BUY  # Buy spot (short perp would be separate order)
            confidence += min(0.1, (premium - self.premium_threshold) / self.premium_threshold)
        
        # Negative premium: perp cheap, long perp + short spot
        elif premium < -self.premium_threshold:
            signal_type = SignalType.SELL  # Sell spot (long perp would be separate order)
            confidence += min(0.1, abs(premium + self.premium_threshold) / self.premium_threshold)
        
        if signal_type is None:
            return None
        
        # Target convergence
        target_premium = self.convergence_target if premium > 0 else -self.convergence_target
