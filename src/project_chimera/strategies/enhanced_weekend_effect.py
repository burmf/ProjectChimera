...
from .performance_mixin import PerformanceMixin, PerformanceDecoratorMixin
...
class EnhancedWeekendEffectStrategy(PerformanceMixin, PerformanceDecoratorMixin, TechnicalStrategy):
    """
    Enhanced Weekend Effect Strategy with Performance Monitoring
    ...
    def __init__(self, config: StrategyConfig):
        ...
        logger.info("Enhanced Weekend Effect Strategy initialized",
    ...
    async def generate_signal(self, market_data: MarketFrame) -> Optional[Signal]:
        """
    ...
    def _should_enter(self, current_time: datetime) -> bool:
        """エントリー条件チェック"""
        return (current_time.weekday() == self.entry_day and 
                current_time.hour >= self.entry_hour)
    
    def _should_exit(self, current_time: datetime) -> bool:
        """エグジット条件チェック"""
        return (current_time.weekday() == self.exit_day and 
                current_time.hour >= self.exit_hour)
    
    async def _create_entry_signal(self, market_data: MarketFrame, current_time: datetime) -> Signal:
        """エントリーシグナル作成"""
        # 信頼度計算（簡易版）
        ...
        signal = Signal(
            ...
            metadata={
                ...
                "technical_context": self.get_latest_indicators() if not indicators.empty else {},
                "market_conditions": {
        ...
        logger.info("Weekend effect entry signal generated",
                   confidence=confidence,
                   price=signal.price,
                   size=signal.size)
    ...
    async def _create_exit_signal(self, market_data: MarketFrame, current_time: datetime) -> Signal:
        """エグジットシグナル作成"""
        ...
        pnl_percentage = ((market_data.last - self.entry_price) / self.entry_price) * 100
        ...
        if hasattr(self.current_position, '_trade_id'):
            asyncio.create_task(self.log_trade_completed(
                trade_id=getattr(self.current_position, '_trade_id'),
        ...
        logger.info("Weekend effect exit signal generated",
                   pnl=pnl,
                   pnl_percentage=pnl_percentage,
                   result=result.value,
    ...
    def _calculate_entry_confidence(self, market_data: MarketFrame, current_time: datetime) -> float:
        """エントリー信頼度計算"""
        ...
        confidence = base_confidence + time_factor + volatility_factor + technical_factor
        return max(0.0, min(1.0, confidence))
    
    def _calculate_position_size(self, market_data: MarketFrame, confidence: float) -> float:
        """ポジションサイズ計算"""
        # 基本サイズ（設定値または1.0）
    ...
    async def run_pdca_cycle_demo(self) -> Dict[str, Any]:
        ...
        session_id = await self.start_pdca_cycle(hypothesis, target_metrics, parameter_changes)
...
def create_enhanced_weekend_effect_strategy(params: Dict[str, Any] = None) -> EnhancedWeekendEffectStrategy:
