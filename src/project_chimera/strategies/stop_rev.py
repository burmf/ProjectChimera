...
class StopReversionStrategy(TechnicalStrategy):
    ...
    def generate_signal(self, market_data: MarketFrame) -> Optional[Signal]:
        ...
        if not market_data.ohlcv_5m or len(market_data.ohlcv_5m) < self.params['lookback_periods']:
            return None
    ...
    def _calculate_volume_spike(self, candles: List) -> float:
        ...
        recent_volumes = [float(c.volume) for c in candles[-self.params['lookback_periods']-1:-1]]
...
def create_stop_reversion_strategy(config: Optional[Dict[str, Any]] = None) -> StopReversionStrategy:
