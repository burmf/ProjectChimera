...
class VolatilityBreakoutStrategy(TechnicalStrategy):
    ...
    def generate_signal(self, market_data: MarketFrame) -> Optional[Signal]:
        ...
        if not market_data.ohlcv_15m or len(market_data.ohlcv_15m) < self.params['bb_period'] + 10:
            return None
        ...
        bb = self.calculate_bollinger_bands(prices, self.params['bb_period'], self.params['bb_std_dev'])
        if bb is None:
            return None
        ...
        if breakout_direction == 'up':
            ...
            stop_loss = current_price - (atr * 1.5) if atr else current_price * (1 - self.params['stop_loss_pct'] / 100)
        ...
        confidence = min(0.8, 0.5 + (squeeze_duration / 20) * 0.2 + (breakout_magnitude / 5) * 0.1)
    ...
    def _is_bollinger_squeeze(self, candles: List, bb: Dict[str, float]) -> bool:
        ...
        if len(candles) < self.params['bb_period'] + self.params['consolidation_periods']:
            return False
        ...
        for i in range(self.params['consolidation_periods']):
            ...
            if historical_bb and historical_bb['bandwidth'] <= self.params['squeeze_threshold']:
                squeeze_count += 1
        
        return squeeze_count >= self.params['consolidation_periods'] * 0.7
    
    def _detect_breakout(self, current_price: float, bb: Dict[str, float], candles: List) -> Optional[str]:
    ...
    def _calculate_breakout_magnitude(self, current_price: float, bb: Dict[str, float]) -> float:
...
def create_volatility_breakout_strategy(config: Optional[Dict[str, Any]] = None) -> VolatilityBreakoutStrategy:
