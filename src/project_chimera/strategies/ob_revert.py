...
class OrderBookMeanReversionStrategy(TechnicalStrategy):
    ...
    def __init__(self, config: StrategyConfig):
        ...
        self.imbalance_threshold = self.params.get('imbalance_threshold', 0.3)  # 30% imbalance
        self.extreme_imbalance = self.params.get('extreme_imbalance', 0.5)  # 50% for strong signals
        self.price_deviation_threshold = self.params.get('price_deviation_threshold', 0.005)  # 0.5%
        self.volume_confirmation = self.params.get('volume_confirmation', True)
        self.volume_threshold = self.params.get('volume_threshold', 1.5)  # 50% above average
        self.spread_max_pct = self.params.get('spread_max_pct', 0.001)  # Max 0.1% spread
    ...
    def validate_config(self) -> None:
        ...
        if self.extreme_imbalance <= self.imbalance_threshold:
            raise ValueError("extreme_imbalance must be greater than imbalance_threshold")
    ...
    def calculate_weighted_imbalance(self, orderbook) -> Optional[float]:
        """
    ...
    def generate_signal(self, market_data: MarketFrame) -> Optional[Signal]:
        ...
        mid_price = (float(market_data.orderbook.best_bid) + float(market_data.orderbook.best_ask)) / 2
        ...
        if imbalance > self.imbalance_threshold and price_deviation > self.price_deviation_threshold:
        ...
        elif imbalance < -self.imbalance_threshold and price_deviation < -self.price_deviation_threshold:
