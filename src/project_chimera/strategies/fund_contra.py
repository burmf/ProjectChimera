...
class FundingContraStrategy(Strategy):
    ...
    def validate_config(self) -> None:
        ...
        self.params.setdefault(
            ...
            getattr(self.strategy_settings, "funding_threshold_pct", 0.03),
        )
        self.params.setdefault(
            "oi_spike_multiplier",
            getattr(self.strategy_settings, "oi_spike_multiplier", 2.0),
        )
        self.params.setdefault(
            "lookback_periods", getattr(self.strategy_settings, "lookback_periods", 24)
        )
        self.params.setdefault(
            "min_funding_history",
            getattr(self.strategy_settings, "min_funding_history", 8),
        )
        self.params.setdefault(
            "max_position_hours",
            getattr(self.strategy_settings, "max_position_hours", 8),
        )
        self.params.setdefault(
            "stop_loss_pct", getattr(self.strategy_settings, "stop_loss_pct", 1.5)
        )
        self.params.setdefault(
            "take_profit_pct", getattr(self.strategy_settings, "take_profit_pct", 2.0)
        )
        self.params.setdefault(
            "funding_momentum_periods",
            getattr(self.strategy_settings, "funding_momentum_periods", 3),
        )
        self.params.setdefault(
            "confidence_base", getattr(self.strategy_settings, "confidence_base", 0.6)
        )
        self.params.setdefault(
            "target_size", getattr(self.strategy_settings, "target_size", 0.04)
        )
    ...
    async def generate_signal(self, market_data: MarketFrame) -> Signal | None:
        ...
        if current_funding > self.params["funding_threshold_pct"] / 100:
            ...
            reasoning = (
                f"Funding contrarian LONG: {current_funding*100:.3f}% funding, OI spike"
            )

        # Calculate confidence based on funding extremity and OI spike
        funding_extremity = abs(current_funding) / (
            self.params["funding_threshold_pct"] / 100
        )
        confidence = min(
            0.9,
            self.params["confidence_base"]
            + (funding_extremity - 1) * 0.1
            + (oi_spike - 2) * 0.05,
        )
    ...
    def _check_oi_spike(self, current_oi: float, oi_history: list[float]) -> float:
        ...
        return (
            spike_multiplier
            if spike_multiplier >= self.params["oi_spike_multiplier"]
            else 0.0
        )
    ...
    def _check_funding_momentum(self, funding_history: list[float]) -> float:
        ...
        if (
            not funding_history
            or len(funding_history) < self.params["funding_momentum_periods"]
        ):
            return 0.0
...
def create_funding_contra_strategy(
    config: dict[str, Any] | None = None,
) -> FundingContraStrategy:
    """Factory function to create FundingContraStrategy"""
    if config is None:
        config = {}

    strategy_config = StrategyConfig(
        name="funding_contrarian", enabled=True, params=config
    )
