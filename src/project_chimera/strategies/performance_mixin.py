...
class PerformanceMixin:
    ...
    def __init__(self, *args, **kwargs):
        ...
        # PDCA管理
        self._pdca_session_id: str | None = None
        self._current_hypothesis = ""
        self._target_metrics = {}

        logger.info(
            "Performance monitoring enabled for strategy",
    ...
    async def log_signal_generated(
        ...
        """シグナル生成ログ"""
        trade_id = (
            f"{self.__class__.__name__}_{datetime.now().strftime('%Y%m%d_%H%M%S_%f')}"
        )
        ...
        for trade in self.performance_logger.trade_history:
            if trade.trade_id == trade_id:
                ...
                trade.pnl_percentage = (
                    (actual_pnl / (trade.entry_price * trade.quantity)) * 100
                    if trade.quantity > 0
                    else 0
                )
    ...
    async def log_pdca_execution(
        self, trades_executed: int, market_conditions: dict[str, Any]
    ) -> None:
        """PDCA実行フェーズログ"""
        ...
        logger.info("PDCA execution phase logged", trades_executed=trades_executed)
    ...
    async def analyze_pdca_performance(self) -> dict[str, Any]:
        ...
        analysis = await self.performance_logger.analyze_performance(
            self.__class__.__name__
        )

        logger.info(
            "PDCA performance analysis completed", strategy=self.__class__.__name__
        )
    ...
    def get_performance_summary(self) -> dict[str, Any]:
        """パフォーマンス要約取得"""
        strategy_metrics = self.performance_logger.strategy_metrics.get(
            self.__class__.__name__
        )
    ...
    def _calculate_spread_bps(self, market_frame: MarketFrame) -> float | None:
        """スプレッドをbpsで計算"""
        try:
    ...
    def measure_execution(func):
        ...
        async def wrapper(self, *args, **kwargs):
            ...
            try:
                ...
                logger.error(
                    "Error in execution", strategy=self.__class__.__name__, error=str(e)
                )
                raise
