...
class BasisArbitrageStrategy(Strategy):
    ...
    def validate_config(self) -> None:
        ...
        self.params.setdefault('max_basis_pct', 5.0)  # Maximum basis (avoid extreme events)
        ...
        self.params.setdefault('convergence_timeframe', 24)  # Expected convergence hours
    ...
    def generate_signal(self, market_data: MarketFrame) -> Optional[Signal]:
        ...
        if not hasattr(market_data, 'spot_price') or not hasattr(market_data, 'perpetual_price'):
            return None
        ...
        convergence_target = spot_price + (perp_price - spot_price) * (1 - self.params['take_profit_pct'] / 100)
    ...
    def _adjust_basis_for_funding(self, basis_pct: float, funding_rate: float) -> float:
        ...
        funding_periods = self.params['convergence_timeframe'] / 8  # Funding every 8 hours
        total_funding_cost = funding_rate * funding_periods * 100  # Convert to percentage
    ...
    def _calculate_funding_alignment(self, basis_pct: float, funding_rate: float) -> float:
    ...
    def _calculate_convergence_probability(self, basis_pct: float, funding_rate: float) -> float:
...
def create_basis_arbitrage_strategy(config: Optional[Dict[str, Any]] = None) -> BasisArbitrageStrategy:
