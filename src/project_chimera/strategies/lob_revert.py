...
class LimitOrderBookReversionStrategy(TechnicalStrategy):
    ...
    def generate_signal(self, market_data: MarketFrame) -> Signal | None:
        ...
        if (
            ...
            or len(market_data.ohlcv_1m) < self.params["rsi_period"] + 5
        ):
            return None
    ...
    def _calculate_orderbook_imbalance(
        ...
        total_bid_size = sum(
            float(level.size)
            for level in orderbook.bids[: self.params["orderbook_levels"]]
        )
        total_ask_size = sum(
            float(level.size)
            for level in orderbook.asks[: self.params["orderbook_levels"]]
        )
    ...
    def _calculate_trade_flow_pressure(
        self, market_data: MarketFrame
    ) -> dict[str, float]:
    ...
    def _calculate_orderbook_depth_ratio(
        self, market_data: MarketFrame
    ) -> float | None:
        ...
        if (
            len(orderbook.bids) < shallow_levels + deep_levels
            or len(orderbook.asks) < shallow_levels + deep_levels
        ):
            return None

        # Calculate shallow liquidity
        shallow_bid = sum(
            float(level.size) for level in orderbook.bids[:shallow_levels]
        )
        shallow_ask = sum(
            float(level.size) for level in orderbook.asks[:shallow_levels]
        )
        ...
        deep_bid = sum(
            float(level.size)
            for level in orderbook.bids[shallow_levels : shallow_levels + deep_levels]
        )
        deep_ask = sum(
            float(level.size)
            for level in orderbook.asks[shallow_levels : shallow_levels + deep_levels]
        )
...
def create_lob_reversion_strategy(
    config: dict[str, Any] | None = None,
) -> LimitOrderBookReversionStrategy:
    """Factory function to create LimitOrderBookReversionStrategy"""
    if config is None:
        config = {}

    strategy_config = StrategyConfig(name="lob_reversion", enabled=True, params=config)
