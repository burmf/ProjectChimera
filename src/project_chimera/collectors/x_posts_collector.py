...
class XPostsCollector:
    ...
    async def start(self, collection_interval_hours: int = 1):
        ...
        logger.info(
            f"Starting X posts collector (interval: {collection_interval_hours}h)"
        )
    ...
    async def _collection_loop(self, interval_hours: int):
        ...
        while self._running:
            try:
                ...
                logger.info(
                    f"Collected {total_posts} X posts in {collection_time:.1f}s"
                )
                ...
                try:
                    ...
                    if post_message and self._is_relevant_post(post_message):
                        ...
                        if post_hash not in self._seen_posts:
                            ...
                            logger.debug(
                                f"Published X post: {post_message.text[:50]}..."
                            )
    ...
    async def _process_post(
        self, post_data: dict, query: XSearchQuery
    ) -> XPostMessage | None:
