...
from .performance_logger import PerformanceLogger, TradeExecution, TradeResult
...
class PDCAAnalyzer:
    ...
    async def run_comprehensive_analysis(self, strategy_name: str) -> dict[str, Any]:
        ...
        # パラメータ最適化分析
        optimization_analysis = await self._analyze_parameter_optimization(
            strategy_name
        )
    ...
    async def _analyze_temporal_patterns(self, strategy_name: str) -> dict[str, Any]:
        ...
        if len(recent_trades) >= 10:
            ...
            trend_analysis = {
                ...
                "performance_trend": (
                    "improving"
                    if recent_avg > all_avg * 1.1
                    else "declining" if recent_avg < all_avg * 0.9 else "stable"
                ),
                "recent_trade_count": len(recent_trades),
                "trend_strength": (
                    abs(recent_avg - all_avg) / abs(all_avg) if all_avg != 0 else 0
                ),
            }
        ...
        return {
            ...
            "performance_consistency": self._calculate_consistency_score(
                daily_performance
            ),
        }
    ...
    async def _analyze_market_regimes(self, strategy_name: str) -> dict[str, Any]:
        ...
        if any(t.spread_bps for t in trades):
            ...
            tight_spread_trades = [
                t for t in spread_trades if t.spread_bps <= median_spread
            ]
            wide_spread_trades = [
                t for t in spread_trades if t.spread_bps > median_spread
            ]

            for label, regime_trades in [
                ("tight_spread", tight_spread_trades),
                ("wide_spread", wide_spread_trades),
            ]:
                if regime_trades:
                    pnls = [t.pnl for t in regime_trades if t.pnl is not None]
                    if pnls:
        ...
        return {
            ...
            "regime_adaptation_score": self._calculate_regime_adaptation_score(
                regime_performance
            ),
            "recommendations": self._generate_regime_recommendations(
                regime_performance, spread_analysis
            ),
        }

    async def _analyze_parameter_optimization(
        ...
        for min_conf, max_conf in confidence_buckets:
            bucket_trades = [
                t for t in trades if min_conf <= t.signal_confidence < max_conf
            ]
            if len(bucket_trades) >= 3:
                ...
                if pnls:
                    confidence_analysis[f"{min_conf}-{max_conf}"] = {
                        ...
                        "win_rate": len([p for p in pnls if p > 0]) / len(pnls),
                        "avg_confidence": sum(
                            t.signal_confidence for t in bucket_trades
                        )
                        / len(bucket_trades),
                    }
        ...
        if confidence_analysis:
            positive_buckets = {
                k: v for k, v in confidence_analysis.items() if v["avg_pnl"] > 0
            }
            if positive_buckets:
                best_bucket = max(
                    positive_buckets.items(), key=lambda x: x[1]["avg_pnl"]
                )
        ...
        if any(t.hold_duration_seconds for t in trades):
            ...
            for label, condition in duration_buckets:
                bucket_trades = [
                    t for t in duration_trades if condition(t.hold_duration_seconds)
                ]
                if bucket_trades:
                    pnls = [t.pnl for t in bucket_trades if t.pnl is not None]
                    if pnls:
                        hold_duration_analysis[label] = {
                            ...
                            "avg_duration_minutes": sum(
                                t.hold_duration_seconds for t in bucket_trades
                            )
                            / len(bucket_trades)
                            / 60,
                        }
    ...
    async def _analyze_risk_metrics(self, strategy_name: str) -> dict[str, Any]:
        ...
        for confidence in [0.95, 0.99]:
            if len(pnls) >= 20:
                ...
                if tail_losses:
                    cvar_calculations[f"CVaR_{int(confidence*100)}"] = sum(
                        tail_losses
                    ) / len(tail_losses)
        ...
        for trade in trades:
            if trade.result == TradeResult.LOSS and trade.pnl is not None:
                ...
                max_consecutive_loss_amount = max(
                    max_consecutive_loss_amount, consecutive_loss_amount
                )
            else:
                consecutive_losses = 0
                consecutive_loss_amount = 0
        ...
        if len(returns) > 1:
            ...
            risk_adjusted_metrics["sharpe_ratio"] = (
                avg_return / return_std if return_std > 0 else 0
            )
            ...
            if downside_returns:
                ...
                risk_adjusted_metrics["sortino_ratio"] = (
                    avg_return / downside_std if downside_std > 0 else 0
                )
        ...
        return {
            ...
            "risk_score": self._calculate_risk_score(
                var_calculations, max_consecutive_losses, risk_adjusted_metrics
            ),
            "risk_recommendations": self._generate_risk_recommendations(
                var_calculations, risk_adjusted_metrics
            ),
        }
    ...
    async def _analyze_execution_efficiency(self, strategy_name: str) -> dict[str, Any]:
        ...
        # 実行時間分析
        signal_times = [
            t.signal_generation_time_ms
            for t in trades
            if t.signal_generation_time_ms > 0
        ]
        execution_times = [
            t.total_execution_time_ms for t in trades if t.total_execution_time_ms > 0
        ]
        ...
        if commissions and pnls:
            ...
            commission_analysis = {
                ...
                "commission_ratio": (
                    total_commission / abs(total_gross_pnl)
                    if total_gross_pnl != 0
                    else 0
                ),
                "commission_efficiency": (
                    "good" if total_commission / abs(total_gross_pnl) < 0.1 else "poor"
                ),
            }
        ...
        return {
            ...
            "bottlenecks": self._identify_performance_bottlenecks(
                timing_analysis, slippage_analysis
            ),
        }

    async def _calculate_overall_score(
        self, strategy_name: str, analyses: dict[str, Any]
    ) -> float:
        """総合スコア計算"""
        scores = {}
        weights = {
    ...
    async def _generate_comprehensive_recommendations(
        ...
        strategy_optimizations = [
            opt for opt in self.optimization_queue if opt.strategy_name == strategy_name
        ]
    ...
    def _calculate_consistency_score(
        self, daily_performance: dict[str, list[float]]
    ) -> float:
        """パフォーマンス一貫性スコア計算"""
    ...
    def _calculate_regime_adaptation_score(
        ...
        positive_regimes = len(
            [r for r in regime_performance.values() if r["avg_pnl"] > 0]
        )
    ...
    def _generate_regime_recommendations(
        self, regime_performance: dict[str, Any], spread_analysis: dict[str, Any]
    ) -> list[str]:
        """レジーム対応推奨事項生成"""
        recommendations = []
        ...
        if regime_performance:
            best_regime = max(regime_performance.items(), key=lambda x: x[1]["avg_pnl"])
            worst_regime = min(
                regime_performance.items(), key=lambda x: x[1]["avg_pnl"]
            )

            if best_regime[1]["avg_pnl"] > 0 and worst_regime[1]["avg_pnl"] < 0:
                recommendations.append(
                    f"{best_regime[0]}環境での取引に特化することを検討"
                )
                recommendations.append(
                    f"{worst_regime[0]}環境での取引を制限することを検討"
                )
    ...
    async def _analyze_parameter_sensitivity(
        ...
        # 相関係数計算
        correlation = (
            np.corrcoef(confidences, pnls)[0, 1] if len(confidences) > 1 else 0
        )
    ...
    def _calculate_risk_score(
        self,
        var_calculations: dict[str, float],
    ...
    def _generate_risk_recommendations(
        ...
        if var_95 < -0.05:
            recommendations.append(
                "ポジションサイズの削減またはストップロス設定の強化が必要"
            )

        sharpe = risk_adjusted_metrics.get("sharpe_ratio", 0)
        if sharpe < 0.5:
            recommendations.append(
                "リスク調整後リターンの改善が必要（目標Sharpe比率 > 1.0）"
            )
    ...
    def _identify_performance_bottlenecks(
        self, timing_analysis: dict[str, Any], slippage_analysis: dict[str, Any]
    ) -> list[str]:
        """パフォーマンスボトルネック特定"""
        bottlenecks = []
...
_pdca_analyzer: PDCAAnalyzer | None = None
