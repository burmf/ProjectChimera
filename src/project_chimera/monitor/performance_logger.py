...
class PerformanceLogger:
    ...
    def __init__(self, log_directory: str = "logs/performance"):
        ...
        self.pdca_file = (
            self.log_directory / f"pdca_{datetime.now().strftime('%Y%m%d')}.jsonl"
        )

        logger.info(
            "PerformanceLogger initialized",
    ...
    async def _update_realtime_stats(self, trade: TradeExecution) -> None:
        ...
        # 時間枠内の取引のみ抽出
        recent_trades = [
            t for t in self.trade_history if current_time - t.timestamp <= time_window
        ]
        ...
        self.real_time_stats = {
            ...
            "avg_execution_time_ms": (
                sum(t.total_execution_time_ms for t in recent_trades) / total_recent
                if total_recent > 0
                else 0
            ),
        }
    ...
    async def analyze_performance(self, strategy_name: str) -> dict[str, Any]:
        ...
        logger.info(
            "Performance analysis completed - Check phase",
            strategy=strategy_name,
            win_rate=metrics.win_rate,
            profit_factor=metrics.profit_factor,
        )
    ...
    async def generate_improvements(
        self, strategy_name: str, analysis_results: dict[str, Any]
    ) -> dict[str, Any]:
        """改善提案生成 (Act フェーズ)"""
        insights = []
        next_actions = []
        ...
        # 勝率分析
        win_rate = len(
            [t for t in strategy_trades if t.result == TradeResult.WIN]
        ) / len(strategy_trades)
        if win_rate < 0.5:
            insights.append(
                f"勝率が低い ({win_rate:.2%}) - エントリー条件の見直しが必要"
            )
        ...
        avg_execution_time = sum(
            t.total_execution_time_ms for t in strategy_trades
        ) / len(strategy_trades)
        if avg_execution_time > 1000:  # 1秒以上
            insights.append(
                f"実行時間が長い ({avg_execution_time:.0f}ms) - 最適化が必要"
            )
            next_actions.append("実行ロジックの最適化")
            optimization_suggestions["execution_optimization"] = (
                "並列処理またはキャッシュ導入を検討"
            )
        ...
        if metrics and metrics.max_drawdown > 0.1:  # 10%以上
            insights.append(
                f"最大ドローダウンが大きい ({metrics.max_drawdown:.2%}) - リスク管理強化が必要"
            )
            next_actions.append("ポジションサイズ調整")
            optimization_suggestions["risk_management"] = (
                "Kelly基準の調整またはストップロス設定の見直し"
            )
        ...
        logger.info(
            "Improvement plan generated - Act phase",
            strategy=strategy_name,
    ...
    async def _calculate_advanced_metrics(self, strategy_name: str) -> dict[str, Any]:
        ...
        hold_durations = [
            ...
            for t in strategy_trades
            if t.hold_duration_seconds is not None
        ]
        avg_hold_duration = (
            sum(hold_durations) / len(hold_durations) if hold_durations else 0
        )
    ...
    async def _generate_optimization_signals(
        ...
        # ボラティリティ最適化
        volatility_trades = [
            t for t in strategy_trades if t.market_volatility is not None
        ]
        if len(volatility_trades) >= 5:
            high_vol_pnl = [
                t.pnl
                for t in volatility_trades
                if t.market_volatility > 0.02 and t.pnl is not None
            ]
            low_vol_pnl = [
                t.pnl
                for t in volatility_trades
    ...
    async def _assess_risk_profile(self, strategy_name: str) -> dict[str, Any]:
        ...
        var_95 = (
            pnls_sorted[int(len(pnls_sorted) * 0.05)]
            if len(pnls_sorted) > 20
            else min(pnls)
        )
        ...
        return {
            ...
            "volatility": (
                sum((p - sum(pnls) / len(pnls)) ** 2 for p in pnls) / len(pnls)
            )
            ** 0.5,
            "risk_level": (
                "high" if var_95 < -0.02 else "medium" if var_95 < -0.01 else "low"
            ),
        }
    ...
    async def _write_to_file(self, file_path: Path, data: dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.error(
                "Failed to write to log file", file=str(file_path), error=str(e)
            )
    ...
    async def get_realtime_dashboard_data(self) -> dict[str, Any]:
        ...
        dashboard_data = {
            ...
            "overall_stats": {
                ...
                "active_pdca_cycles": len(
                    [p for p in self.pdca_log if p.phase != PerformancePhase.ACT]
                ),
            },
    ...
    async def export_analysis_report(self, strategy_name: str | None = None) -> str:
        """分析レポート出力"""
        ...
        if strategy_name:
            report_data = {
                ...
                "pdca_log": [
                    asdict(p) for p in self.pdca_log if p.strategy_name == strategy_name
                ],
            }
...
async def initialize_performance_logging(
