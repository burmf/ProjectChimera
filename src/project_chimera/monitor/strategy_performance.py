...
class StrategyPerformanceTracker:
    ...
    def _init_database(self) -> None:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_pnl_strategy ON realtime_pnl(strategy_id)"
            )
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                for row in cursor.fetchall():
                    ...
                    existing_trades = [
                        t
                        for t in self.trade_records[trade.strategy_id]
                        if t.signal_id == trade.signal_id
                    ]
                ...
                total_loaded = sum(
                    len(trades) for trades in self.trade_records.values()
                )
    ...
    def _calculate_strategy_stats_sync(self, strategy_id: str) -> None:
        ...
        if len(returns) > 1:
            ...
            stats.sharpe_ratio = (
                (avg_return - self.risk_free_rate / 252) / std_return
                if std_return > 0
                else 0.0
            )
    ...
    async def record_signal_generated(
        self, signal: Signal, market_frame: MarketFrame
    ) -> str:
        """Record when a signal is generated (not yet executed)"""
        strategy_id = getattr(signal, "strategy_id", None) or getattr(
        ...
        metadata_field = getattr(signal, "metadata", None) or getattr(
    ...
    async def record_trade_entry(
        ...
        trade = TradeRecord(
            ...
            side=(
                str(signal.signal_type.value).lower()
                if hasattr(signal, "signal_type")
                else getattr(signal, "action", "buy")
            ),
        ...
        logger.info(
            f"Trade entry recorded: {signal_id} - {strategy_id} {side} {size_usd:.2f} USD"
        )
        return signal_id
    ...
    async def record_trade_exit(
        ...
        logger.info(
            f"Trade exit recorded: {signal_id} - P&L: ${trade.pnl_usd:.2f} ({trade.pnl_pct:.2f}%)"
        )
        return trade
    ...
    async def _calculate_strategy_stats(self, strategy_id: str) -> None:
        ...
        if negative_returns:
            ...
            stats.sortino_ratio = (
                (np.mean(returns) - self.risk_free_rate / 252) / downside_std
                if downside_std > 0
                else 0.0
            )
        ...
        # Calmar ratio
        annualized_return = (
            stats.total_pnl_pct * (252 / len(trades)) if len(trades) > 0 else 0.0
        )
        stats.calmar_ratio = (
            annualized_return / stats.max_drawdown_pct
            if stats.max_drawdown_pct > 0
            else 0.0
        )

        # Execution metrics
        stats.avg_holding_time_hours = np.mean(
            [
                t.holding_time_seconds / 3600
                for t in trades
                if t.holding_time_seconds > 0
            ]
        )
        ...
        # Recent performance
        now = datetime.now()
        stats.last_24h_pnl_usd = sum(
            t.pnl_usd for t in trades if (now - t.entry_time).days < 1
        )
        stats.last_7d_pnl_usd = sum(
            t.pnl_usd for t in trades if (now - t.entry_time).days < 7
        )
        stats.last_30d_pnl_usd = sum(
            t.pnl_usd for t in trades if (now - t.entry_time).days < 30
        )
    ...
    async def _record_realtime_pnl(
        self, strategy_id: str, unrealized_pnl: float
    ) -> None:
    ...
    def get_strategy_stats(self, strategy_id: str) -> StrategyStats | None:
    ...
    def get_performance_summary(self) -> dict[str, Any]:
        ...
        total_volume = sum(
            stats.total_volume_usd for stats in self.strategy_stats.values()
        )

        strategy_count = len(self.strategy_stats)
        avg_win_rate = (
            np.mean([stats.win_rate for stats in self.strategy_stats.values()])
            if strategy_count > 0
            else 0.0
        )
        ...
        return {
            ...
            "best_strategy": (
                max(self.strategy_stats.items(), key=lambda x: x[1].total_pnl_usd)[0]
                if self.strategy_stats
                else None
            ),
            "worst_strategy": (
                min(self.strategy_stats.items(), key=lambda x: x[1].total_pnl_usd)[0]
                if self.strategy_stats
                else None
            ),
...
_performance_tracker: StrategyPerformanceTracker | None = None
