...
class TradingMetricsCollector:
    ...
    def _init_metrics(self):
        ...
        self.metrics["pnl_total"] = PrometheusMetric(
            "chimera_pnl_total_usd", "gauge", "Total P&L in USD"
        )
        ...
        self.metrics["dd_pct"] = PrometheusMetric(
            "chimera_drawdown_percent", "gauge", "Current drawdown percentage"
        )
        ...
        self.metrics["orders_total"] = PrometheusMetric(
            "chimera_orders_total", "counter", "Total number of orders placed"
        )

        self.metrics["orders_filled"] = PrometheusMetric(
            "chimera_orders_filled_total", "counter", "Total number of orders filled"
        )

        self.metrics["equity_value"] = PrometheusMetric(
            "chimera_equity_value_usd", "gauge", "Current portfolio equity value in USD"
        )

        self.metrics["system_uptime"] = PrometheusMetric(
            "chimera_system_uptime_seconds", "gauge", "System uptime in seconds"
        )
        ...
        self.metrics["api_errors"] = PrometheusMetric(
            "chimera_api_errors_total", "counter", "Total API errors encountered"
        )
    ...
    def record_signal(self, strategy: str, symbol: str):
        ...
        self.metrics["strategy_signals"].inc(
            ...
            try:
                ...
                if random.random() < 0.05:  # 5% chance
                    reason = random.choice(
                        ["position_limit", "drawdown_limit", "correlation_limit"]
                    )
...
class PrometheusExporter:
    ...
    def start(self, simulate: bool = True):
        ...
        self.running = True
        logger.info(
            f"Prometheus exporter running on http://localhost:{self.port}/metrics"
        )
...
def main():
    ...
    parser.add_argument(
        "--no-simulate", action="store_true", help="Disable metrics simulation"
    )
