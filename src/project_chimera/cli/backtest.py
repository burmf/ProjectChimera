...
class VectorizedBacktester:
        ...
        for i, row in enumerate(data):
            ...
            if timestamp_str:
                try:
                    ...
                    timestamps[i] = (
                        float(timestamp_str) if timestamp_str.isdigit() else i
                    )
            else:
                timestamps[i] = i
    ...
    def backtest_strategy(
        self, data: MarketData, strategy: StrategyType
    ) -> BacktestResult:
    ...
    def _execute_vectorized_backtest(
        self, data: MarketData, signals: np.ndarray
    ) -> BacktestResult:
        ...
        for i in range(1, n_points):
            ...
            if signal != 0 and not in_position:
                ...
                transaction_cost = position_size * (
                ...
                trades.append(
                    {
                        ...
                        "pnl": pnl,
                        "return_pct": (
                            pnl / (abs(position[i - 1]) * entry_price)
                            if position[i - 1] != 0
                            else 0
                        ),
                    }
    ...
    def _calculate_metrics(
        self, data: MarketData, equity: np.ndarray, trades: list[dict]
    ) -> BacktestResult:
        ...
        annual_return = (
            (final_equity / initial_equity) ** (1 / max(duration_years, 1 / 365)) - 1
            if duration_years > 0
            else 0
        )
        ...
        if len(equity_returns) > 0 and volatility > 0:
            sharpe_ratio = (
                np.mean(equity_returns) * 252 - self.config.risk_free_rate
            ) / volatility
        ...
        if len(equity_returns) > 0:
            ...
            expected_shortfall = (
                np.mean(equity_returns[equity_returns <= var_95])
                if np.any(equity_returns <= var_95)
                else var_95
            )
        else:
    ...
    def _netflow_flt_strategy(self, data: MarketData) -> np.ndarray:
        ...
        for i in range(10, len(data.closes)):
            ...
            if vol_ratio > 2:  # High volume = netflow
                price_change = (data.closes[i] - data.closes[i - 1]) / data.closes[
                    i - 1
                ]
    ...
    def _vol_term_flip_strategy(self, data: MarketData) -> np.ndarray:
        ...
        for i in range(20, len(data.closes)):
            ...
            vol_threshold = np.percentile(
                [np.std(data.closes[j - 20 : j]) for j in range(20, i)], 70
            )
    ...
    def _social_jump_strategy(self, data: MarketData) -> np.ndarray:
        ...
        for i in range(10, len(data.closes)):
            vol_z = (data.volumes[i] - np.mean(data.volumes[i - 10 : i])) / np.std(
                data.volumes[i - 10 : i]
            )
    ...
    def _alt_beta_rot_strategy(self, data: MarketData) -> np.ndarray:
        ...
        # Delayed momentum
        for i in range(20, len(data.closes)):
            momentum = (data.closes[i - 5] - data.closes[i - 20]) / data.closes[
                i - 20
            ]  # Lagged momentum
            if momentum > 0.02:
                signals[i] = 1
...
def print_performance_summary(results: dict[StrategyType, BacktestResult]) -> None:
    ...
    print(
        f"{'Strategy':<20} {'Return':<10} {'Sharpe':<8} {'MaxDD':<8} {'Trades':<8} {'Win%':<8} {'Calmar':<8}"
    )
    print("-" * 100)

    for strategy_type, result in sorted_results:
        print(
            f"{strategy_type.value:<20} {result.total_return:>8.1%} {result.sharpe_ratio:>7.2f} "
    ...
    sharpe_ratios = [
        r.sharpe_ratio for r in results.values() if not np.isnan(r.sharpe_ratio)
    ]

    print(
        f"Summary: {len(results)} strategies | Avg Return: {np.mean(total_returns):.1%} | "
...
def main():
    ...
    parser.add_argument(
        "--strats",
        default="all",
        help="Strategies to test (all, vol_breakout, mini_momo, etc.)",
    )
    parser.add_argument(
    ...
    try:
        ...
        if data.length >= 100000:
            ...
            if minutes <= 2.0:
                print(
                    f"✅ Performance target met: {minutes:.1f} min < 2.0 min for 100k+ rows"
                )
            else:
                print(
                    f"⚠️  Performance target missed: {minutes:.1f} min > 2.0 min for 100k+ rows"
                )
