...
try:
    ...
    from project_chimera.cli.backtest import (
        BacktestConfig,
        MarketData,
        StrategyType,
        VectorizedBacktester,
...
class OptimizationConfig:
    ...
    timeout: float | None = None  # Max time in seconds
    random_seed: int = 42
    ...
    storage: str | None = None  # For distributed optimization
...
class ParameterSpace:
    ...
    parameters: dict[str, tuple[int | float, int | float]]  # (min, max) tuples
...
class StrategyOptimizer:
    ...
    def _generate_parameterized_signals(
        self, strategy_type: StrategyType, params: dict[str, Any]
    ) -> "np.ndarray":
    ...
    def _vol_breakout_with_params(self, params: dict[str, Any]) -> "np.ndarray":
        ...
        std = np.array(
            [np.std(closes[i : i + window]) for i in range(len(closes) - window + 1)]
        )
    ...
    def _mini_momentum_with_params(self, params: dict[str, Any]) -> "np.ndarray":
        ...
        for i in range(window, len(closes)):
            ...
            z_score = (closes[i] - np.mean(recent_prices)) / (
    ...
    def _default_strategy_with_params(
        self, strategy_type: StrategyType, params: dict[str, Any]
    ) -> "np.ndarray":
    ...
    def optimize_strategy(self, strategy_type: StrategyType) -> OptimizationResult:
        ...
        if OPTUNA_AVAILABLE:
            ...
            # Find best result
            for _, (params, value) in enumerate(
                zip(param_combinations, results, strict=False)
            ):
                all_trials.append(
                    {
                        "params": params,
        ...
        if best_params:
            ...
            best_result = self.backtester._execute_vectorized_backtest(
                self.data, signals
            )
        ...
        trials_per_second = (
            len(all_trials) / optimization_time if optimization_time > 0 else 0
        )
        ...
        logger.info(
            f"Completed {strategy_type.value}: {completed_trials}/{self.config.n_trials} trials, "
    ...
    def optimize_all_strategies(self) -> dict[StrategyType, OptimizationResult]:
        ...
        logger.info(
            f"Starting optimization for {len(self.parameter_spaces)} strategies..."
        )
        start_time = time.time()

        results = {}

        for strategy_type in self.parameter_spaces:
            try:
        ...
        logger.info(
            f"Completed optimization for {len(results)} strategies in {total_time:.2f}s"
        )
...
def print_optimization_summary(results: dict[StrategyType, OptimizationResult]) -> None:
    ...
    sorted_results = sorted(
        results.items(), key=lambda x: x[1].best_value, reverse=True
    )

    print(
        f"{'Strategy':<20} {'Best Value':<12} {'Trials':<8} {'Success%':<9} {'Time(s)':<8} {'TPS':<8}"
    )
    print("-" * 100)

    for strategy_type, result in sorted_results:
        success_rate = (
            (result.completed_trials / result.n_trials * 100)
            if result.n_trials > 0
            else 0
        )

        print(
            f"{strategy_type.value:<20} {result.best_value:>10.3f} "
    ...
    print(
        f"Summary: {len(results)} strategies | {total_completed}/{total_trials} trials completed | "
...
def main():
    ...
    parser.add_argument(
        "--strats",
        default="all",
        help="Strategies to optimize (all, vol_breakout, etc.)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--jobs", type=int, default=-1, help="Number of parallel jobs (-1 = all cores)"
    )
