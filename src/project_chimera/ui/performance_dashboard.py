...
class PerformanceDashboard:
    ...
    def _render_sidebar(self):
        ...
        if strategies:
            selected = st.sidebar.selectbox(
                ...
                index=(
                    ...
                    else strategies.index(st.session_state.selected_strategy) + 1
                ),
            )
            st.session_state.selected_strategy = (
                None if selected == "Overview" else selected
            )
        else:
        ...
        if st.session_state.auto_refresh:
            st.session_state.refresh_interval = st.sidebar.slider(
                "Refresh Interval (sec)", 10, 120, st.session_state.refresh_interval
            )
        ...
        if st.session_state.selected_strategy and st.sidebar.button(
        ...
        dashboard_data = asyncio.run(
            self.performance_logger.get_realtime_dashboard_data()
        )
        st.sidebar.metric(
            "Total Trades", dashboard_data["overall_stats"]["total_trades"]
        )
        st.sidebar.metric(
            "Active Strategies", dashboard_data["overall_stats"]["total_strategies"]
        )
        st.sidebar.metric(
            "Uptime (min)", f"{dashboard_data['session_info']['uptime_minutes']:.1f}"
        )
    ...
    def _render_overview_dashboard(self):
        ...
        with col3:
            total_pnl = sum(
                perf["total_pnl"]
                for perf in dashboard_data["strategy_performance"].values()
            )
        ...
        with col4:
            avg_win_rate = (
                ...
                / len(dashboard_data["strategy_performance"])
    ...
    def _render_strategy_dashboard(self):
        ...
        with st.spinner("Running comprehensive analysis..."):
            analysis = asyncio.run(
                self.pdca_analyzer.run_comprehensive_analysis(strategy_name)
            )
    ...
    def _render_comprehensive_analysis(self, analysis):
        ...
        # 総合スコア
        st.subheader(
            f"🎯 Overall Performance Score: {analysis['overall_score']:.1f}/100"
        )
        ...
        score_color = (
            ...
            else "orange" if analysis["overall_score"] >= 50 else "red"
        )
        ...
        with tab4:
            self._render_optimization_analysis(
                analysis["analyses"]["parameter_optimization"]
            )
        ...
        with tab6:
            self._render_execution_analysis(
                analysis["analyses"]["execution_efficiency"]
            )
        ...
        for i, rec in enumerate(analysis["recommendations"][:5]):  # 上位5つ
            ...
            st.write(
                f"   *Expected: {rec['expected_improvement']} (Confidence: {rec['confidence']})*"
            )
            st.write("---")
    ...
    def _render_basic_analysis(self, basic_analysis):
        ...
        trades = [
            t
            for t in self.performance_logger.trade_history
            if t.strategy_name == st.session_state.selected_strategy
    ...
    def _render_temporal_analysis(self, temporal_analysis):
        ...
        # 最適取引時間
        if (
            "best_trading_hours" in temporal_analysis
            and temporal_analysis["best_trading_hours"]
        ):
        ...
        # トレンド分析
        if (
            "trend_analysis" in temporal_analysis
            and temporal_analysis["trend_analysis"]
        ):
            trend = temporal_analysis["trend_analysis"]
            trend_icon = {"improving": "📈", "declining": "📉", "stable": "➡️"}
            st.write(
                f"📊 **Performance Trend:** {trend_icon.get(trend['performance_trend'], '❓')} {trend['performance_trend'].title()}"
            )
    ...
    def _render_regime_analysis(self, regime_analysis):
        ...
        if "volatility_regimes" in regime_analysis:
            ...
            if regimes:
                ...
                fig.update_layout(
                    xaxis_title="Volatility Regime", yaxis_title="Average P&L ($)"
                )
                st.plotly_chart(fig, use_container_width=True)
    ...
    def _render_optimization_analysis(self, optimization_analysis):
        ...
        if "confidence_analysis" in optimization_analysis:
            ...
            if conf_data:
                ...
                fig = px.bar(
                    x=conf_ranges, y=avg_pnls, title="Performance by Confidence Level"
                )
                fig.update_layout(
                    xaxis_title="Confidence Range", yaxis_title="Average P&L ($)"
                )
                st.plotly_chart(fig, use_container_width=True)

        # 最適信頼度閾値
        if (
            "optimal_confidence_threshold" in optimization_analysis
            and optimization_analysis["optimal_confidence_threshold"]
        ):
            st.write(
                f"🎯 **Optimal Confidence Threshold:** {optimization_analysis['optimal_confidence_threshold']:.2f}"
            )
        ...
        if "optimization_recommendations" in optimization_analysis:
            ...
            if recs:
                ...
                for rec in recs:
                    st.write(
                        f"   • **{rec['parameter_name']}**: {rec['current_value']} → {rec['recommended_value']}"
                    )
    ...
    def _render_risk_analysis(self, risk_analysis):
        ...
        with col2:
            if "risk_adjusted_metrics" in risk_analysis:
                for metric_name, metric_value in risk_analysis[
                    "risk_adjusted_metrics"
                ].items():
                    st.metric(
                        metric_name.replace("_", " ").title(), f"{metric_value:.2f}"
                    )
        ...
        if "risk_score" in risk_analysis:
            ...
            score_color = (
                "green" if risk_score >= 70 else "orange" if risk_score >= 50 else "red"
            )
    ...
    def _render_pdca_management(self, strategy_name):
        ...
        pdca_sessions = [
            p
            for p in self.performance_logger.pdca_log
        ...
        with st.expander("Start New PDCA Cycle"):
            hypothesis = st.text_area(
                "Hypothesis",
                placeholder="例: 信頼度閾値を0.7に上げることで勝率が向上する",
            )
            ...
            with col1:
                ...
                target_profit_factor = st.number_input(
                    "Target Profit Factor", min_value=0.0, value=2.0, step=0.1
                )

            with col2:
                param_name = st.text_input(
                    "Parameter to Change", placeholder="confidence_threshold"
                )
            ...
            if st.button("Start PDCA Cycle") and hypothesis:
                ...
                parameter_changes = (
                    {param_name: param_value} if param_name and param_value else {}
                )

                # 非同期でPDCAサイクル開始（実際の実装では適切な非同期処理が必要）
                st.success(
                    "PDCA Cycle started! (Note: This is a demo - actual implementation needed)"
                )
        ...
        if pdca_sessions:
            ...
            for session in pdca_sessions[-5:]:  # 最新5件
                with st.expander(
                    f"{session.timestamp.strftime('%Y-%m-%d %H:%M')} - {session.phase.value}"
                ):
    ...
    def _render_trade_history_details(self, strategy_name):
            ...
            trade_data.append(
                {
                    ...
                    "Exit Price": (
                        f"${trade.exit_price:.4f}" if trade.exit_price else "N/A"
                    ),
                    ...
                    "Hold Time (min)": (
                        f"{trade.hold_duration_seconds/60:.1f}"
                        if trade.hold_duration_seconds
                        else "N/A"
                    ),
                    "Exec Time (ms)": (
                        f"{trade.total_execution_time_ms:.0f}"
                        if trade.total_execution_time_ms
                        else "N/A"
                    ),
                }
            )
        ...
        if pnl_data:
            timestamps, pnls = zip(*pnl_data, strict=False)
            cumulative_pnl = pd.Series(pnls).cumsum().tolist()

            fig = go.Figure()
            fig.add_trace(
                go.Scatter(
                    x=timestamps, y=cumulative_pnl, mode="lines", name="Cumulative P&L"
                )
            )
            fig.update_layout(
    ...
    def _render_realtime_stats(self, realtime_stats):
        ...
        with col4:
            st.metric(
                "Avg Exec Time", f"{realtime_stats['avg_execution_time_ms']:.0f}ms"
            )
    ...
    def _export_strategy_report(self):
        ...
        try:
            ...
            report_path = asyncio.run(
                self.performance_logger.export_analysis_report(strategy_name)
            )
