...
try:
    from ..monitor.strategy_performance import (
        StrategyStats,
        TradeRecord,
    ...
    from src.project_chimera.monitor.strategy_performance import (
        StrategyStats,
        TradeRecord,
        get_performance_tracker,
    )
...
class StrategyPerformanceAPI:
    ...
    def get_strategy_details(self, strategy_id: str) -> StrategyStats | None:
    ...
    def toggle_strategy(self, strategy_id: str, enabled: bool) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Strategy {strategy_id} {'enabled' if enabled else 'disabled'}"
            )
            return True
        except Exception as e:
...
def create_strategy_overview_chart(strategies: dict[str, StrategyStats]) -> go.Figure:
    ...
    sharpe_colors = [
        "darkgreen" if sr >= 1 else "orange" if sr >= 0 else "red"
...
def create_strategy_performance_chart(stats: StrategyStats) -> go.Figure:
            ...
            gauge={
                ...
                "bar": {
                    "color": (
                        ...
                        else "orange" if stats.sharpe_ratio >= 0 else "red"
                    )
                },
                "steps": [
    ...
    cumulative_pnl = np.cumsum(
        np.random.normal(stats.total_pnl_usd / 30, stats.daily_pnl_std, 30)
    )
    ...
    execution_values = [
        ...
        (
            (stats.total_commission_usd / stats.total_volume_usd * 10000)
        ...
        data.append(
            {
                ...
                "Holding Time": (
                    f"{trade.holding_time_seconds/3600:.1f}h"
...
def strategy_dashboard():
    ...
    if strategy_names:
        selected_strategy = st.sidebar.selectbox(
            "Select Strategy", strategy_names, index=0
        )
    else:
    ...
    if selected_strategy:
        ...
        if new_enabled != current_enabled:
            if strategy_api.toggle_strategy(selected_strategy, new_enabled):
                st.sidebar.success(
                    f"Strategy {'enabled' if new_enabled else 'disabled'}"
                )
            else:
    ...
    tab1, tab2, tab3, tab4 = st.tabs(
        ["📈 Overview", "🔍 Strategy Details", "📋 Trade History", "⚙️ Settings"]
    )
    ...
    with tab1:
        ...
        if all_strategies:
            ...
            avg_win_rate = np.mean(
                [stats.win_rate for stats in all_strategies.values()]
            )
            best_strategy = max(
                all_strategies.items(), key=lambda x: x[1].total_pnl_usd
            )
            active_strategies = len(
                [s for s in all_strategies.values() if s.total_trades > 0]
            )

            with col1:
                st.metric(
                    "Total P&L", f"${total_pnl:.2f}", delta=f"${total_pnl*0.1:.2f}"
                )  # Mock delta
            ...
            with col5:
                st.metric(
                    "Active Strategies", f"{active_strategies}/{len(all_strategies)}"
                )
    ...
    with tab2:
        ...
        if selected_strategy and selected_strategy in all_strategies:
            ...
            if (
                selected_strategy in open_positions
                and open_positions[selected_strategy]
            ):
                ...
                for pos in open_positions[selected_strategy]:
                    ...
                    unrealized_pnl = (
                        (current_price - pos.entry_price) * pos.size_native
                        if pos.side == "buy"
                        else (pos.entry_price - current_price) * pos.size_native
                    )
        ...
        if selected_strategy:
            ...
            with col2:
                trade_status = st.selectbox(
                    "Status Filter", ["All", "Filled", "Open", "Cancelled"]
                )
            ...
            if trades:
                ...
                with col1:
                    ...
                    if not trades_df.empty:
                        ...
                        pnl_values = [
                            float(pnl.replace("$", "").replace(",", ""))
                            for pnl in trades_df["P&L (USD)"]
                            if pnl != "$0.00"
                        ]

                        if pnl_values:
                            st.metric(
                                "Avg P&L per Trade", f"${np.mean(pnl_values):.2f}"
                            )
    ...
    with tab4:
        ...
        if selected_strategy:
            ...
            if st.button("Update Strategy Settings", type="primary"):
                ...
                # Log the changes
                logger.info(
                    f"Strategy {selected_strategy} settings updated: "
