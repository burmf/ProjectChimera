...
class TechnicalAnalyzer:
    ...
    def calculate_all_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(
                f"Calculated {len([col for col in df.columns if any(x in col.lower() for x in ['rsi', 'macd', 'bb', 'sma', 'ema', 'atr'])])} technical indicators"
            )
    ...
    def _add_momentum_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        stoch_data = ta.stoch(
            df["high"], df["low"], df["close"], k=stoch_config["k"], d=stoch_config["d"]
        )
        if stoch_data is not None:
            df["stoch_k"] = stoch_data[
                f'STOCHk_{stoch_config["k"]}_{stoch_config["d"]}_3'
            ]
            df["stoch_d"] = stoch_data[
                f'STOCHd_{stoch_config["k"]}_{stoch_config["d"]}_3'
            ]
    ...
    def _add_trend_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if supertrend_data is not None and isinstance(
    ...
    def _add_volatility_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        bb_data = ta.bbands(
            df["close"], length=bb_config["length"], std=bb_config["std"]
        )
        ...
        # Volatility Index
        df["volatility"] = (
            ta.true_range(df["high"], df["low"], df["close"]).rolling(20).mean()
        )
        ...
        if rsi_value >= rsi_config["overbought"]:
            signals.append(
                TechnicalSignal(
                    ...
                    signal_type="sell",
                    strength=min(
                        (rsi_value - rsi_config["overbought"])
                        / (100 - rsi_config["overbought"]),
                        1.0,
                    ),
                    value=rsi_value,
            ...
            signals.append(
                TechnicalSignal(
                    ...
                    signal_type="buy",
                    strength=min(
                        (rsi_config["oversold"] - rsi_value) / rsi_config["oversold"],
                        1.0,
                    ),
                    value=rsi_value,
    ...
    def get_market_regime(self, df: pd.DataFrame) -> dict[str, str | float]:
        """
