...
class DynamicKellyResult:
    ...
    def is_valid(self) -> bool:
        ...
        return (
            0.0 <= self.kelly_fraction <= 1.0 and
            0.0 <= self.ewma_win_rate <= 1.0 and
...
class DynamicKellyCalculator:
    ...
    def __init__(self, config: Optional[DynamicKellyConfig] = None):
        ...
        self.last_calculation: Optional[DynamicKellyResult] = None
    ...
    def add_trade_result(self, return_pct: float, timestamp: Optional[datetime] = None) -> None:
        ...
        if self.vol_ewma == 0.0:
            ...
            self.vol_ewma = self.config.ewma_alpha * abs_return + (1 - self.config.ewma_alpha) * self.vol_ewma
        
        # Update EWMA estimates
        self._update_ewma_estimates(return_pct)
        
        logger.debug(f"Added trade: {return_pct:.4f}%, EWMA win rate: {self.ewma_win_rate:.3f}")
    ...
    def _update_ewma_estimates(self, return_pct: float) -> None:
        ...
        if len(self.trade_returns) > 1:
            ...
            if self.ewma_return_variance == 0.0:
                ...
                self.ewma_return_variance = alpha * variance_update + (1 - alpha) * self.ewma_return_variance
    ...
    def calculate_dynamic_kelly(self) -> DynamicKellyResult:
        ...
        if len(self.trade_returns) < self.config.min_sample_size:
            return self._get_default_result(f"Insufficient data: {len(self.trade_returns)} < {self.config.min_sample_size}")
        ...
        logger.info(f"Dynamic Kelly: {adjusted_kelly:.3f} (raw: {raw_kelly:.3f}, vol_adj: {vol_adjustment:.3f}, conf: {confidence:.3f})")
    ...
    def _calculate_confidence_score(self, returns: List[float]) -> float:
        ...
        if n >= 20:
            ...
            consistency = 1.0 - min(1.0, abs(recent_mean - older_mean) / (abs(older_mean) + 0.01))
        ...
        winrate_score = min(1.0, wr_distance_from_center * 4.0)  # Peak confidence at 62.5% or 37.5% win rate
    ...
    def _filter_outliers(self, returns: List[float]) -> List[float]:
        ...
        if outliers_removed > 0:
            logger.debug(f"Filtered {outliers_removed} outliers from {len(returns)} trades")
    ...
    def simulate_performance(self, num_simulations: int = 1000) -> Dict[str, float]:
        ...
        return {
            ...
            "probability_of_loss": sum(1 for eq in final_equities if eq < 1.0) / len(final_equities)
        }