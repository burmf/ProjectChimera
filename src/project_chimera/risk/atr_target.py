...
class ATRTargetResult:
    ...
    def is_valid(self) -> bool:
        ...
        return (
            ...
            and self.current_atr > 0
            and self.confidence_score >= 0.3
            and self.daily_vol_estimate > 0
        )
...
class ATRTargetController:
    ...
    def __init__(self, config: ATRTargetConfig | None = None):
        ...
        self.last_calculation: ATRTargetResult | None = None
    ...
    def add_price_data(self, ohlcv: OHLCV) -> None:
        ...
        if len(self.price_history) >= self.config.atr_periods:
            ...
            if self.daily_vol_ema == 0.0:
                ...
                self.daily_vol_ema = (
                    alpha * daily_vol + (1 - alpha) * self.daily_vol_ema
                )
            ...
            logger.debug(
                f"ATR: {atr:.6f}, Daily Vol: {daily_vol:.4f}, Price: {ohlcv.close:.2f}"
            )
    ...
    def calculate_target_position_size(self, current_price: float) -> ATRTargetResult:
        ...
        vol_target_ratio = (
            self.daily_vol_ema / self.config.target_daily_vol
        ...
        logger.info(
            f"ATR Target: {adjusted_position_size:.3f} position, "
    ...
    def _update_vol_regime(self) -> None:
        ...
        longer_vol = np.mean(
            vol_list[-20:] if len(vol_list) >= 20 else vol_list
        )  # Last 20 days or all
    ...
    def _calculate_confidence_score(self) -> float:
        ...
        if len(self.atr_history) >= 5:
            ...
            atr_cv = (
                np.std(recent_atr) / np.mean(recent_atr)
                if np.mean(recent_atr) > 0
                else float("inf")
            )
    ...
    def get_volatility_forecast(self, days_ahead: int = 1) -> dict[str, float]:
        ...
        for day in range(days_ahead):
            ...
            ) * current_forecast + mean_reversion_factor * long_term_avg
    ...
    def simulate_vol_targeting(
        self, test_prices: list[OHLCV], target_vol: float | None = None
    ) -> dict[str, Any]:
        ...
        results = {
            ...
            "vol_target_hit_rate": sum(
                1 for v in achieved_vols if abs(v - target_vol) / target_vol < 0.2
            )
            / len(achieved_vols),
