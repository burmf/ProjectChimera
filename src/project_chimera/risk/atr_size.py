...
class ATRPositionSizer:
    ...
    def calculate_position_size(
        ...
        target_met = (
            self.target_config.min_position_size <= base_position_size <= 
    ...
    def _update_vol_regime(self, atr_value: float, price: float, timestamp: datetime) -> None:
        """Update volatility regime tracking"""
        ...
        self.atr_history = [
            (ts, vol) for ts, vol in self.atr_history 
            if ts >= cutoff_time
        ]
