...
class EquityCache:
    ...
    def __init__(
        ...
        self._stats_cache: Optional[EquityStats] = None
    ...
    def add_equity_point(
        ...
        if (self.persistence_file and 
            self.update_count % self.auto_save_interval == 0):
            self.save_to_file()
    ...
    def _insert_point(self, point: EquityPoint) -> None:
        ...
        if (pos < len(self.equity_points) and 
            self.equity_points[pos].timestamp == point.timestamp):
    ...
    def get_drawdown(self, timestamp: Optional[datetime] = None) -> float:
        """
    ...
    def get_equity_at_time(self, timestamp: datetime) -> Optional[float]:
        ...
        if pos == 0:
            # Before first point
            return self.equity_points[0].equity if self.equity_points else self.initial_equity
    ...
    def _get_peak_equity_before(self, timestamp: Optional[datetime] = None) -> float:
    ...
    def backfill_equity_curve(
        self, 
        historical_points: List[Tuple[datetime, float, float]]
    ) -> None:
        """
    ...
    def _recalculate_peak(self) -> None:
        ...
        self.peak_equity = max(self.initial_equity, 
                             max(p.equity for p in self.equity_points))
