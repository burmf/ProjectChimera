...
class KellyCalculator:
    ...
    def calculate_kelly(self, returns: list[float] | None = None) -> KellyResult:
        """
    ...
    def _calculate_confidence(
        self, returns: list[float], kelly_fraction: float
    ) -> float:
        """
    ...
    def simulate_kelly_performance(
        ...
        if len(equity_curve) > 1:
            returns_series = [
                equity_curve[i + 1] / equity_curve[i] - 1
                for i in range(len(equity_curve) - 1)
            ]
            mean_return = statistics.mean(returns_series)
            std_return = (
                statistics.stdev(returns_series) if len(returns_series) > 1 else 0
            )
