...
class DrawdownState:
    ...
    cooldown_until: datetime | None  # Trading halt until this time
...
class DrawdownManager:
    ...
    def update_equity(
        ...
        self.equity_history = [
            (ts, eq) for ts, eq in self.equity_history if ts >= cutoff_time
        ]
        ...
        tier, multiplier, cooldown_delta = self._calculate_tier_and_multiplier(
            drawdown_pct
        )

        # Set cooldown if tier changed to more severe
        cooldown_until = None
        if tier.value != self.current_state.tier.value and cooldown_delta > timedelta(
            0
        ):
    ...
    def _calculate_tier_and_multiplier(
        self, drawdown_pct: float
    ) -> tuple[DrawdownTier, float, timedelta]:
        ...
        if drawdown_pct >= critical_thresh:
            ...
            return (
                DrawdownTier.CAUTION,
                self.caution_multiplier,
                self.caution_cooldown,
            )
        else:
    ...
    def simulate_recovery_time(
        self, target_return_rate: float = 0.02
    ) -> datetime | None:
        """
        ...
        days_needed = math.log(1 + required_total_return) / math.log(
            1 + target_return_rate
        )
