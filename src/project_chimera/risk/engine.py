...
from .equity_cache import EquityCache, EquityStats
...
class RiskEngine:
    ...
    def __init__(
        ...
        equity_cache_file: Optional[str] = "data/equity_cache.json"
    ):
        ...
        self.equity_cache = EquityCache(
            persistence_file=equity_cache_file,
    ...
    def calculate_position_size(
        ...
        can_trade = (
            self.drawdown_mgr.can_trade() and
        ...
        if not can_trade:
            return self._create_no_trade_decision(
                kelly_result, atr_result, drawdown_state,
        ...
        if base_sizes:
            ...
            if total_weight > 0:
                weighted_size = sum(size * conf for _, size, conf in base_sizes) / total_weight
            else:
        ...
        estimated_vol = self.atr_sizer.estimate_daily_portfolio_vol(
            position_size_pct=final_size,
            price_volatility=atr_result.price_volatility
        )
        
        estimated_dd_risk = self._estimate_drawdown_risk(final_size, atr_result.price_volatility)
        ...
        reasoning = self._generate_reasoning(
            signal, kelly_result, atr_result, drawdown_state, 
            final_size, primary_constraint
        )
    ...
    def update_trade_result(
        ...
        logger.info(f"Updated trade result: {return_pct:.4f}% return, portfolio: ${portfolio_value:.2f}")
    ...
    def _calculate_portfolio_risk_limit(self, current_positions: Dict[str, float]) -> float:
    ...
    def _estimate_drawdown_risk(self, position_size: float, price_volatility: float) -> float:
    ...
    def _generate_reasoning(
        ...
        # Signal assessment
        parts.append(f"Signal: {signal.signal_type.value} with {signal.confidence:.2f} confidence")
        
        # Kelly assessment
        if kelly.sample_size >= 20:
            parts.append(f"Kelly: {kelly.fraction:.3f} (win rate: {kelly.win_rate:.2f})")
        else:
            parts.append(f"Kelly: insufficient data ({kelly.sample_size} trades)")
        
        # ATR assessment
        parts.append(f"ATR sizing: {atr.position_size_pct:.3f} (vol: {atr.price_volatility:.3f})")
        
        # Drawdown impact
        if drawdown.tier != DrawdownTier.NORMAL:
            parts.append(f"Drawdown: {drawdown.tier.value} ({drawdown.drawdown_pct:.2f}%), size reduced by {(1-drawdown.position_multiplier)*100:.0f}%")
        
        # Final decision
        parts.append(f"Final: {final_size:.3f} limited by {constraint.replace('_', ' ')}")
