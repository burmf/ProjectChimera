...
from .atr_target import ATRTargetConfig, ATRTargetController, ATRTargetResult
from .dd_guard import DDGuardConfig, DDGuardState, DDGuardSystem, DDGuardTier
...
class UnifiedRiskEngine:
    ...
    def __init__(
        self, config: UnifiedRiskConfig | None = None, initial_equity: float = 1.0
    ):
        ...
        # State tracking
        self.last_decision: UnifiedRiskDecision | None = None
        self.decision_count: int = 0

        logger.info(
            f"Unified Risk Engine initialized with equity: {initial_equity:.4f}"
        )
    ...
    async def calculate_position_size(
        ...
        logger.info(
            f"Risk decision: {vol_limited_size:.3f} position, "
    ...
    def update_trade_result(
        ...
        logger.info(
            f"Updated trade result: {return_pct:.4f}% return, "
    ...
    def _calculate_base_size(
        ...
        if (
            kelly_result.is_valid()
            and kelly_result.sample_size >= self.config.kelly_min_trades
        ):
            options.append(
                ("kelly", kelly_result.kelly_fraction, kelly_result.confidence_score)
            )

        # ATR sizing (always available)
        if atr_result.is_valid():
            options.append(
                ("atr", atr_result.position_size_pct, atr_result.confidence_score)
            )
    ...
    def _apply_volatility_limits(
        self, position_size: float, atr_result: ATRTargetResult, portfolio_value: float
    ) -> float:
    ...
    def _calculate_leverage(
        self, position_size: float, signal_confidence: float
    ) -> float:
    ...
    def _estimate_portfolio_volatility(
        self, position_size: float, asset_volatility: float
    ) -> float:
    ...
    def _identify_constraint(
        ...
        if dd_multiplier < 1.0:
            dd_tier = (
                ...
                else "warning" if dd_multiplier <= 0.5 else "caution"
            )
    ...
    def _generate_reasoning(
        ...
        signal_strength = (
            ...
            else "moderate" if signal.confidence > 0.4 else "weak"
        )
        parts.append(
            f"{signal_strength} {signal.signal_type.value} signal ({signal.confidence:.2f})"
        )
        ...
        if (
            ...
            parts.append(
                f"Kelly sizing: {kelly_result.kelly_fraction:.3f} "
                f"(WR: {kelly_result.ewma_win_rate:.2f}, {kelly_result.sample_size} trades)"
            )
        elif sizing_method == "atr":
            parts.append(
                f"ATR sizing: {atr_result.position_size_pct:.3f} "
        ...
        if dd_state.tier != DDGuardTier.NORMAL:
            parts.append(
                f"DD guard: {dd_state.tier.value} "
    ...
    def _create_no_trade_decision(
        self, dd_state: DDGuardState, reason: str
    ) -> UnifiedRiskDecision:
        ...
        return UnifiedRiskDecision(
            ...
            can_trade=False,
            confidence=0.0,
            kelly_result=self.kelly_calc.last_calculation
            or DynamicKellyResult(
                kelly_fraction=0.0,
                ewma_win_rate=0.5,
                ewma_avg_win=0.0,
                ewma_avg_loss=0.0,
            ...
            atr_result=self.atr_controller.last_calculation
            or ATRTargetResult(
                position_size_pct=0.0,
                current_atr=0.0,
    ...
    async def health_check(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "last_decision": (
                    self.last_decision.primary_constraint
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        engine_stats = {
            ...
            "last_decision_time": (
                self.last_decision.kelly_result.last_updated
                if self.last_decision
                else None
            ),
            "config": {
    ...
    def hot_reload_config(self, new_config: UnifiedRiskConfig) -> None:
        ...
        self.dd_guard.config.warning_cooldown_hours = (
            new_config.dd_warning_cooldown_hours
        )
        self.dd_guard.config.critical_cooldown_hours = (
            new_config.dd_critical_cooldown_hours
        )

        logger.info(
            f"Configuration hot-reloaded: "
