...
class DDGuardState:
    ...
    def time_until_cooldown_ends(self) -> Optional[timedelta]:
...
class DDGuardSystem:
    ...
    def __init__(self, config: Optional[DDGuardConfig] = None, initial_equity: float = 1.0):
    ...
    def update_equity(self, new_equity: float, timestamp: Optional[datetime] = None) -> DDGuardState:
        ...
        if new_equity > self.peak_equity * (1 + self.config.peak_update_threshold):
            ...
            logger.debug(f"New equity peak: {self.peak_equity:.4f} (from {old_peak:.4f})")
        ...
        logger.debug(f"DD Guard: Equity={new_equity:.4f}, DD={current_dd:.3f}, "
    ...
    def _handle_tier_transition(self, new_tier: DDGuardTier, drawdown: float, timestamp: datetime) -> None:
        ...
        tier_severity = {DDGuardTier.NORMAL: 0, DDGuardTier.CAUTION: 1, 
                        DDGuardTier.WARNING: 2, DDGuardTier.CRITICAL: 3}
        ...
        if is_recovery and time_in_current_tier < min_time_in_tier:
            logger.debug(f"DD Guard: Preventing tier recovery too soon "
        ...
        if is_recovery:
            ...
            if recovery_needed < self.config.recovery_threshold:
                logger.debug(f"DD Guard: Insufficient recovery progress "
                            f"({recovery_needed:.2f} < {self.config.recovery_threshold:.2f})")
                return
        
        # Execute tier transition
        logger.info(f"DD Guard: Tier transition {old_tier.value} → {new_tier.value} "
        ...
        if new_tier == DDGuardTier.WARNING:
            self.cooldown_until = timestamp + timedelta(hours=self.config.warning_cooldown_hours)
            logger.warning(f"DD Guard: 10% DD reached - reducing position size to 50%, "
                          f"cooldown until {self.cooldown_until}")
        elif new_tier == DDGuardTier.CRITICAL:
            self.cooldown_until = timestamp + timedelta(hours=self.config.critical_cooldown_hours)
            logger.critical(f"DD Guard: 20% DD reached - TRADING HALT for 24 hours, "
                           f"cooldown until {self.cooldown_until}")
    ...
    def force_tier_change(self, new_tier: DDGuardTier, reason: str = "Manual override") -> None:
        ...
        if new_tier == DDGuardTier.WARNING:
            self.cooldown_until = datetime.now() + timedelta(hours=self.config.warning_cooldown_hours)
        elif new_tier == DDGuardTier.CRITICAL:
            self.cooldown_until = datetime.now() + timedelta(hours=self.config.critical_cooldown_hours)
        else:
    ...
    def reset_peak(self, new_peak: Optional[float] = None) -> None:
        ...
        logger.warning(f"DD Guard: Peak reset from {old_peak:.4f} to {self.peak_equity:.4f}")
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        if len(self.equity_history) > 1:
            ...
            stats = {
                ...
                # Equity curve stats
                "total_return": (self.current_equity / self.equity_history[0] - 1) if self.equity_history[0] != 0 else 0,
                "volatility": np.std(equity_returns) * np.sqrt(252),  # Annualized
                "sharpe_estimate": np.mean(equity_returns) / np.std(equity_returns) * np.sqrt(252) if np.std(equity_returns) > 0 else 0,
                
                # Tier statistics
                "tier_changes": len(self.tier_history),
                "time_in_normal": tier_times[DDGuardTier.NORMAL].total_seconds() / 3600,
                "time_in_caution": tier_times[DDGuardTier.CAUTION].total_seconds() / 3600,
                "time_in_warning": tier_times[DDGuardTier.WARNING].total_seconds() / 3600,
                "time_in_critical": tier_times[DDGuardTier.CRITICAL].total_seconds() / 3600,
            }
            
            if self.cooldown_until:
                stats["cooldown_remaining_hours"] = (self.cooldown_until - datetime.now()).total_seconds() / 3600
    ...
    def simulate_drawdown_response(self, equity_curve: List[float]) -> Dict[str, Any]:
        ...
        for i, equity in enumerate(equity_curve):
            ...
            # Track statistics
            if i > 0 and simulation_results[i]["tier"] != simulation_results[i-1]["tier"]:
                tier_changes += 1
            
            max_position_reduction = max(max_position_reduction, 1.0 - state.position_multiplier)
        ...
        return {
            ...
            "final_tier": simulation_results[-1]["tier"] if simulation_results else "unknown",
            "drawdown_breaches": {
                "caution": sum(1 for r in simulation_results if r["drawdown"] >= self.config.caution_threshold),
                "warning": sum(1 for r in simulation_results if r["drawdown"] >= self.config.warning_threshold),
                "critical": sum(1 for r in simulation_results if r["drawdown"] >= self.config.critical_threshold)
            }
        }
