...
class DataCache:
    ...
    async def get(self, key: str, data_type: str) -> Any | None:
...
class AsyncDataFeed:
            ...
            async for ticker in self.adapter.stream_ticker(symbol):
                ...
                if ticker.timestamp:
                    ...
                    if len(self.metrics["latency_samples"]) > 100:
                        self.metrics["latency_samples"] = self.metrics[
                            "latency_samples"
                        ][-100:]
            ...
            try:
                ...
                if self.metrics["last_update"]:
                    ...
                    if time_since_update > timedelta(seconds=60):
                        logger.warning(
                            "No data received for 60 seconds, checking health"
                        )
    ...
    async def snapshot(self, symbol: str) -> MarketFrame | None:
        """
        ...
        orderbook = (
            await self.cache.get(symbol, "orderbook") if self.enable_orderbook else None
        )
    ...
    def get_metrics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "latency_median_ms": (
                sorted(latency_samples)[len(latency_samples) // 2]
                if latency_samples
                else None
            ),
            "latency_p95_ms": (
