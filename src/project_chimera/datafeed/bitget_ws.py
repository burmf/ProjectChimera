...
class BitgetWebSocketFeed(ExchangeAdapter):
    ...
    def __init__(self, name: str, config: Dict[str, Any]):
        ...
        if self.sandbox:
            self.spot_ws_url = "wss://ws.bitgetapi.com/spot/v1/stream"
            self.mix_ws_url = "wss://ws.bitgetapi.com/mix/v1/stream"
            self.rest_base_url = self.exchange_config.demo_rest_url
        else:
            self.spot_ws_url = self.exchange_config.spot_ws_url
            self.mix_ws_url = self.exchange_config.mix_ws_url
            self.rest_base_url = self.exchange_config.rest_base_url
    ...
    async def connect(self) -> None:
        ...
        if self._is_circuit_breaker_active():
            raise ConnectionError("Circuit breaker active - too many connection failures")
        ...
        try:
            ...
            logger.error("bitget_connection_failed", error=str(e), failures=self.connection_failures)
            raise
    ...
    async def _connect_websockets_with_retry(self) -> None:
        ...
        try:
            ...
            logger.info("bitget_websockets_connected", 
    ...
    async def subscribe_ticker(self, symbol: str) -> None:
        ...
        logger.info("subscribed_ticker", symbol=symbol, futures=self._is_futures_symbol(symbol))
    ...
    async def subscribe_orderbook(self, symbol: str, levels: int = 20) -> None:
        ...
        logger.info("subscribed_orderbook", symbol=symbol, levels=levels, type=depth_type)
    ...
    async def _subscribe_spot_channel(self, channel: str) -> None:
        ...
        try:
            subscribe_msg = {
                "op": "subscribe",
                "args": [{"instType": "sp", "channel": channel, "instId": "default"}]
            }
    ...
    async def _subscribe_mix_channel(self, channel: str) -> None:
        ...
        try:
            subscribe_msg = {
                "op": "subscribe", 
                "args": [{"instType": "mc", "channel": channel, "instId": "default"}]
            }
    ...
    async def _process_message(self, message: str, ws_type: str, receive_time: float) -> None:
        ...
        try:
            ...
            if "arg" in data and "data" in data:
                ...
                if handler_name in self.message_handlers:
                    await self.message_handlers[handler_name](data["data"], data["arg"])
                else:
    ...
    async def _monitor_latency(self) -> None:
        ...
        while self.status == ConnectionStatus.CONNECTED:
            try:
                ...
                # Log latency metrics
                logger.info("ws_latency_metrics",
    ...
    async def _reconnect_spot(self) -> None:
        ...
        for attempt in range(self.exchange_config.max_reconnect_attempts):
            try:
                ...
                logger.warning("spot_reconnect_failed", attempt=attempt + 1, error=str(e))
        
        logger.error("spot_reconnect_exhausted", max_attempts=self.exchange_config.max_reconnect_attempts)
    ...
    async def _reconnect_mix(self) -> None:
        ...
        for attempt in range(self.exchange_config.max_reconnect_attempts):
            try:
                ...
                logger.warning("mix_reconnect_failed", attempt=attempt + 1, error=str(e))
        
        logger.error("mix_reconnect_exhausted", max_attempts=self.exchange_config.max_reconnect_attempts)
    ...
    async def _handle_orderbook(self, data: List[Dict], arg: Dict) -> None:
        ...
        for book in data:
            ...
            if symbol:
                ...
                logger.debug("orderbook_updated", symbol=symbol, 
        ...
        for rate in data:
            ...
            if symbol:
                funding_rate = FundingRate(
                    ...
                    next_funding_time=datetime.fromisoformat(rate.get("fundingTime", "")),
                    timestamp=datetime.now(timezone.utc)
                )
    ...
    async def get_historical_klines(
        ...
        try:
            ...
            # Bitget REST endpoint for historical data
            endpoint = f"/api/spot/v1/market/candles"
            params = {
                "symbol": symbol,
            ...
            for candle in data.get("data", []):
                ohlcv = OHLCV(
                    ...
                    timestamp=datetime.fromtimestamp(int(candle[0]) / 1000, timezone.utc)
                )
                klines.append(ohlcv)
    ...
    async def health_check(self) -> bool:
        ...
        try:
            ...
            response = await self.http_client.get("/api/spot/v1/public/time", timeout=5)
            response.raise_for_status()
            return True
