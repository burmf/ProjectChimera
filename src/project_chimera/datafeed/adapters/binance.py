...
class BinanceAdapter(ExchangeAdapter):
    ...
    async def subscribe_funding(self, symbol: str) -> None:
        ...
        logger.debug(f"Funding rate subscription not implemented for spot symbol {symbol}")
    ...
    async def _handle_messages(self, symbol: str, ws: websockets.WebSocketServerProtocol) -> None:
    ...
    async def get_ticker(self, symbol: str) -> Optional[Ticker]:
        ...
        try:
            response = await self.http_client.get(f"/api/v3/ticker/24hr", params={'symbol': symbol})
            response.raise_for_status()
            data = response.json()
    ...
    def is_connected(self) -> bool:
        ...
        return self.status == ConnectionStatus.CONNECTED and self.http_client is not None
