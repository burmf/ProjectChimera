...
class BitgetEnhancedAdapter(ExchangeAdapter):
    ...
    async def _subscribe_mix_channel(self, channel: str) -> None:
        ...
        try:
            subscribe_msg = {
                "op": "subscribe",
                "args": [
                    {"instType": "UMCBL", "channel": channel, "instId": "default"}
                ],
            }
    ...
    async def get_historical_klines(
        ...
        try:
            ...
            endpoint = (
                ...
                if not self._is_futures_symbol(symbol)
                else "/api/mix/v1/market/candles"
            )
    ...
    async def get_funding_rate(self, symbol: str) -> FundingRate | None:
        ...
        try:
            response = await self.http_client.get(
                "/api/mix/v1/market/current-fundRate", params={"symbol": symbol}
            )
            response.raise_for_status()
    ...
    async def get_open_interest(self, symbol: str) -> float | None:
        ...
        try:
            response = await self.http_client.get(
                "/api/mix/v1/market/open-interest", params={"symbol": symbol}
            )
            response.raise_for_status()
    ...
    def is_connected(self) -> bool:
        ...
        return (
            ...
            and (
                (self.spot_ws and not self.spot_ws.closed)
                or (self.mix_ws and not self.mix_ws.closed)
            )
        )
