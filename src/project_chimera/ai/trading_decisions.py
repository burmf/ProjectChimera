...
class MarketContext:
    ...
    def get_sentiment_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "sentiment_range": (
                [min(sentiments), max(sentiments)] if sentiments else [0, 0]
            ),
        }
    ...
    def get_news_summary(self) -> dict[str, Any]:
        ...
        relevances = [
            n.relevance_score for n in self.recent_news if n.relevance_score is not None
        ]
...
class TradingDecisionProcessor:
    ...
    async def update_news_data(self, news_msg: NewsMessage):
        ...
        for context in self.market_contexts.values():
            ...
            # Remove old news
            context.recent_news = [
                n for n in context.recent_news if n.timestamp > cutoff_time
            ]
    ...
    async def update_x_posts_data(self, post_msg: XPostMessage):
        ...
        for context in self.market_contexts.values():
            ...
            context.recent_x_posts = [
                p for p in context.recent_x_posts if p.timestamp > cutoff_time
            ]

        logger.debug(f"Updated X posts data: {post_msg.text[:30]}...")

    async def update_portfolio_context(
        self, symbol: str, positions: list[dict], portfolio_value: float, pnl: float
    ):
    ...
    def get_market_context(self, symbol: str) -> MarketContext | None:
        ...
        if context:
            ...
            if age_minutes > self.context_timeout_minutes:
                logger.warning(
                    f"Market context for {symbol} is {age_minutes:.1f} minutes old"
                )
    ...
    def prepare_1min_decision_context(self, symbol: str) -> dict[str, Any] | None:
    ...
    def prepare_1hour_strategy_context(
        ...
        strategy_context = {
            ...
            "technical_analysis": {
                symbol: {
                    ...
                    "price_history": context.price_history[
                        -50:
                    ],  # More history for strategy
                    "technical_indicators": context.technical_indicators,
                }
                for symbol, context in zip(symbols, valid_contexts, strict=False)
            },
            "news_sentiment": {
            ...
            "portfolio_state": {
                "total_positions": sum(
                    len(c.current_positions) for c in valid_contexts
                ),
    ...
    def _aggregate_sentiment(self, posts: list[XPostMessage]) -> dict[str, Any]:
        ...
        return {
            "avg_sentiment": sum(sentiments) / len(sentiments) if sentiments else 0.0,
            "avg_engagement": (
                sum(engagements) / len(engagements) if engagements else 0.0
            ),
