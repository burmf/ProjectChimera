...
class AIDecisionConfig:
    ...
    openai_api_key: str
    model_name: str = "o3-mini"  # Use o3-mini for cost efficiency, upgrade to o3 for production
    max_tokens: int = 1000
...
class AIDecisionResult:
    ...
    success: bool
    decision: Optional[AIDecisionMessage]
...
class OpenAIDecisionEngine:
    ...
    async def _decision_1min_loop(self):
        ...
        while self._running:
            try:
                    ...
                    try:
                        ...
                        if self._should_make_1min_decision(symbol):
                            ...
                            if result.success and result.decision:
                                ...
                                logger.info(f"1-min decision for {symbol}: {result.decision.action} "
                                          f"(confidence: {result.decision.confidence:.2f})")
                            else:
                                logger.warning(f"Failed 1-min decision for {symbol}: {result.error_message}")
                ...
                logger.debug(f"1-min decision loop completed: {decisions_made} decisions in {loop_time:.1f}s")
            ...
            try:
                ...
                if symbols:
                    ...
                    if result.success and result.decision:
                        ...
                        logger.info(f"1-hour strategy update: {result.decision.action} "
                                  f"(confidence: {result.decision.confidence:.2f})")
                    else:
                        logger.warning(f"Failed 1-hour strategy: {result.error_message}")
    ...
    async def _make_1min_decision(self, symbol: str) -> AIDecisionResult:
        ...
        try:
            ...
            # Parse response
            decision = self._parse_ai_response(response.choices[0].message.content, "1min_trade", symbol)
    ...
    async def _make_1hour_strategy(self, symbols: List[str]) -> AIDecisionResult:
        ...
        try:
            # Prepare context
            context_data = self.decision_processor.prepare_1hour_strategy_context(symbols)
            if not context_data:
                return AIDecisionResult(
                    success=False,
                    decision=None,
            ...
            response = await self.openai_client.chat.completions.create(
                model=self.config.model_name,
                messages=[
                    {"role": "system", "content": "You are a senior cryptocurrency fund manager."},
                    {"role": "user", "content": prompt}
                ],
            ...
            # Parse response
            decision = self._parse_ai_response(response.choices[0].message.content, "1hour_strategy", "PORTFOLIO")
    ...
    def _build_1min_prompt(self, context_data: Dict[str, Any]) -> str:
        ...
        # Format context data
        market_context = TradingPrompts.format_market_context(context_data["market_context"])
        price_data = TradingPrompts.format_price_data(context_data["price_data"])
        orderbook_data = TradingPrompts.format_orderbook_data(context_data["orderbook_data"])
...
"""
    ...
    def _build_1hour_prompt(self, context_data: Dict[str, Any]) -> str:
        ...
        technical_analysis = json.dumps(context_data["technical_analysis"], indent=2, default=str)
        news_sentiment = json.dumps(context_data["news_sentiment"], indent=2, default=str)
    ...
    def _parse_ai_response(self, response_text: str, decision_type: str, symbol: str) -> Optional[AIDecisionMessage]:
        ...
        try:
            ...
            if confidence < self.config.min_confidence_threshold:
                logger.info(f"AI confidence {confidence:.2f} below threshold {self.config.min_confidence_threshold}")
    ...
    def _calculate_api_cost(self, usage) -> float:
        ...
        model_pricing = self.model_pricing.get(self.config.model_name, {"input": 0.001, "output": 0.004})
    ...
    def _update_cost_tracking(self, cost: float):
        ...
        self.api_call_history = [
            (timestamp, cost) for timestamp, cost in self.api_call_history
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_strategy_time": self.last_1hour_strategy.timestamp.isoformat() if self.last_1hour_strategy else None,
