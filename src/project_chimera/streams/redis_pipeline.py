...
from .message_types import MessageEncoder, StreamMessage
...
class StreamConsumer:
                    ...
                    for message_id, fields in stream_messages:
                        try:
                            ...
                            await redis_client.xack(
                                self.config.name, self.config.consumer_group, message_id
                            )
    ...
    async def _process_message(
        self, redis_client: Redis, message_id: str, fields: dict
    ):
...
async def create_market_data_consumer(
    pipeline: RedisStreamPipeline, handler: Callable
) -> StreamConsumer:
...
async def create_news_consumer(
    pipeline: RedisStreamPipeline, handler: Callable
) -> StreamConsumer:
...
async def create_ai_decision_consumer(
    pipeline: RedisStreamPipeline, handler: Callable
) -> StreamConsumer:
    """Create consumer for AI decisions stream"""
    return pipeline.register_consumer("ai_decisions", "execution", "executor", handler)
