...
class CircuitBreaker:
    ...
    def can_execute(self) -> bool:
        ...
        if (self.last_failure_time and 
            time.time() - self.last_failure_time > self.pause_duration):
            self.reset()
    ...
    def record_success(self):
        ...
        if self.consecutive_failures > 0:
            logger.info(f"Circuit breaker: Reset after {self.consecutive_failures} failures")
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        if self.is_open and self.last_failure_time:
            remaining_pause = max(0, self.pause_duration - (time.time() - self.last_failure_time))
    ...
    async def post(self, url: str, **kwargs) -> 'MockResponse':
        ...
        return MockResponse({
            ...
            'data': {
                ...
                'clientOid': kwargs.get('json', {}).get('clientOid', 'mock_client_id')
            }
        })
...
class BitgetExecutionEngine:
    ...
    async def place_order(self, order: Order) -> bool:
        ...
        try:
            ...
            http_client = aiohttp.ClientSession() if AIOHTTP_AVAILABLE else MockHTTPClient()
    ...
    async def cancel_order(self, order_id: str) -> bool:
        ...
        try:
            request_path = f'/api/spot/v1/trade/cancel-order'
        ...
        try:
            ...
            if data.get('event') == 'trade':
                ...
                if fill.order_id in self.orders:
                    ...
                    if order.filled_size >= order.size * 0.99:  # Allow for minor rounding
                        order.status = OrderStatus.FILLED
                        self.metrics['orders_filled'] += 1
                
                logger.info(f"Fill received: {fill.symbol} {fill.side.value} {fill.size} @ {fill.price}")
    ...
    def get_health(self) -> Dict[str, Any]:
        ...
        return {
            'status': 'healthy' if self.running and self.circuit_breaker.can_execute() else 'unhealthy',
