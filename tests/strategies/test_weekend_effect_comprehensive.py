...
from project_chimera.strategies.enhanced_weekend_effect import (
    EnhancedWeekendEffectStrategy as WeekendEffectStrategy,
)
...
class TestWeekendEffectStrategy:
    ...
    def strategy(self, basic_config):
        ...
        with patch(
            "project_chimera.strategies.weekend_effect.get_strategy_config"
        ) as mock_get_config:
    ...
    def test_config_validation_success(self, basic_config):
        ...
        with patch(
            ...
            for attr in [
                ...
                setattr(
                    mock_strategy_settings, attr, basic_config.params.get(attr, 0.7)
                )
            mock_get_config.return_value = mock_strategy_settings
    ...
    def test_config_validation_invalid_friday_hour(self, basic_config):
        ...
        with patch(
            ...
            with pytest.raises(
                ValueError, match="friday_entry_hour must be between 0-23"
            ):
    ...
    def test_config_validation_invalid_monday_hour(self, basic_config):
        ...
        with patch(
            ...
            with pytest.raises(
                ValueError, match="monday_exit_hour must be between 0-23"
            ):
                WeekendEffectStrategy(basic_config)
    ...
    async def test_generate_signal_friday_buy_time(self, strategy, sample_market_frame):
        ...
        with (
            patch.object(strategy, "_is_friday_buy_time", return_value=True),
            patch.object(strategy, "_is_monday_sell_time", return_value=False),
            patch.object(strategy, "_check_volatility_conditions", return_value=True),
            patch.object(strategy, "_check_market_conditions", return_value=True),
        ):
            ...
            assert (
                len(signals) >= 0
            )  # May or may not generate signals based on implementation

    @pytest.mark.asyncio
    async def test_generate_signal_monday_sell_time(
        self, strategy, sample_market_frame
    ):
        ...
        # Mock additional methods that might be called
        with (
            patch.object(strategy, "_is_friday_buy_time", return_value=False),
            patch.object(strategy, "_is_monday_sell_time", return_value=True),
            patch.object(strategy, "_check_volatility_conditions", return_value=True),
            patch.object(strategy, "_check_market_conditions", return_value=True),
        ):
    ...
    @pytest.mark.asyncio
    async def test_generate_signal_disabled_strategy(
        self, strategy, sample_market_frame
    ):
    ...
    def test_parameter_fallbacks(self):
        ...
        with patch(
            ...
            mock_strategy_settings = MagicMock()
            mock_strategy_settings.enable_friday_buy = (
                True  # This should be overridden by config
            )
