...
from project_chimera.domains.market import MarketFrame, Ticker, Signal, SignalType, SignalStrength
...
class TestWeekendEffectStrategy:
    ...
    async def test_generate_signal_friday_buy_time(self, strategy, sample_market_frame):
        ...
        with patch.object(strategy, '_is_friday_buy_time', return_value=True), patch.object(strategy, '_is_monday_sell_time', return_value=False), patch.object(strategy, '_check_volatility_conditions', return_value=True), patch.object(strategy, '_check_market_conditions', return_value=True):
    ...
    async def test_generate_signal_monday_sell_time(self, strategy, sample_market_frame):
        ...
        # Mock additional methods that might be called
        with patch.object(strategy, '_is_friday_buy_time', return_value=False), patch.object(strategy, '_is_monday_sell_time', return_value=True), patch.object(strategy, '_check_volatility_conditions', return_value=True), patch.object(strategy, '_check_market_conditions', return_value=True):
