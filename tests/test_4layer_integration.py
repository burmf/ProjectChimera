...
from project_chimera.ai.openai_decision_engine import OpenAIDecisionEngine, AIDecisionConfig
from project_chimera.orchestrator_4layer import (
    ProjectChimera4LayerOrchestrator, OrchestratorConfig
)
...
class TestNewsRSSCollector:
    ...
    async def test_article_filtering(self, redis_pipeline):
        ...
        assert collector._is_relevant_article(relevant_news) == True
        assert collector._is_relevant_article(irrelevant_news) == False
...
class TestXPostsCollector:
    ...
    async def test_crypto_relevance_detection(self, redis_pipeline):
        ...
        assert collector._is_crypto_related("Bitcoin is pumping today!") == True
        assert collector._is_crypto_related("ETH breaking out above resistance") == True
        assert collector._is_crypto_related("Crypto market looking bullish") == True
        
        # Test non-crypto content
        assert collector._is_crypto_related("Weather is nice today") == False
        assert collector._is_crypto_related("Sports game was exciting") == False
...
class TestOrchestrator4Layer:
    ...
    async def test_orchestrator_startup_sequence(
        ...
        assert orchestrator._startup_complete == True
